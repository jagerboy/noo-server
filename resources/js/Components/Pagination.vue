...
const emit = defineEmits(['change-per-page', 'changePerPage']);
...
function changePerPage() {
  const newPerPage = selectedPerPage.value;
  emit('change-per-page', newPerPage);
  emit('changePerPage', newPerPage);

  // Check if parent component is listening to change-per-page event
  const vnodeProps = instance?.vnode?.props || {};
  const hasListener = Boolean(
    vnodeProps['onChange-per-page'] ||
    vnodeProps['onChangePerPage'] ||
    vnodeProps['onChange-per-page-once']
  );
  ...
  // Fallback standalone behavior using Laravel link URL or current URL:
  let baseUrl = window.location.href;
  const linkWithUrl = props.links?.find((l) => l.url);
  if (linkWithUrl?.url) {
    baseUrl = linkWithUrl.url;
  }
  const currentUrl = new URL(baseUrl, window.location.origin);
  currentUrl.searchParams.set('per_page', String(newPerPage));
      ...
      <div class="flex items-center gap-2 border-l border-[#E5E7EB] pl-4">
        ...
        <div class="relative inline-flex items-center">
          <select
            ...
            class="pl-2.5 pr-7 py-1 text-[11px] font-semibold bg-[#F8FAFC] border border-[#CBD5E1] rounded-[6px] focus:ring-1 focus:ring-[#2563EB] focus:border-[#2563EB] cursor-pointer text-[#1E293B] shadow-2xs"
          >
