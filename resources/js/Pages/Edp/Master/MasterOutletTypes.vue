...
<template>
  <EdpLayout>
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3.5 sm:p-4 md:p-5 rounded-xl border border-[#E5E7EB] shadow-xs">
        <div>
          <h1 class="text-lg sm:text-xl md:text-[22px] font-bold text-[#111827] tracking-tight flex items-center gap-2">
            <span>Master Outlet Types / Channel Tipe Toko</span>
          </h1>
      ...
      <!-- FILTER BAR -->
      <div class="bg-white p-3 sm:p-3.5 rounded-xl border border-[#E5E7EB] shadow-xs flex items-center gap-2.5">
        <input
          type="text"
          v-model="search"
        ...
        />
        <button @click="applyFilters" class="px-3 py-1.5 text-[11.5px] font-semibold text-white bg-[#374151] rounded-lg cursor-pointer">Cari</button>
        <button @click="resetFilters" class="px-2.5 py-1.5 text-[10.5px] font-medium text-rose-500 hover:text-rose-700 hover:underline cursor-pointer">Reset</button>
      </div>
      ...
      <div class="bg-white rounded-xl border border-[#E5E7EB] shadow-xs overflow-hidden">
        <table class="w-full text-left">
          <thead class="bg-[#F9FAFB] border-b border-[#E5E7EB] font-semibold text-[#475569] text-[11.5px] uppercase">
            <tr>
              <th class="px-3 py-3">Kode Tipe Outlet</th>
              <th class="px-3 py-3">Deskripsi / Tipe Channel</th>
          ...
          <tbody class="divide-y divide-[#E5E7EB] text-[12.5px] leading-[17px]">
            <tr v-for="t in outletTypes" :key="t.id" class="hover:bg-emerald-50/20 transition">
              <td class="px-3 py-2.5 font-mono font-bold text-[#111827] text-[12px]">{{ t.code }}</td>
              <td class="px-3 py-2.5 text-[#374151] font-bold text-[13px]">{{ t.description }}</td>
              <td class="px-3 py-2.5">
                <span v-if="t.is_active" class="px-2 py-0.5 text-[10.5px] font-semibold text-emerald-700 bg-emerald-100 rounded-full">AKTIF</span>
                <span v-else class="px-2 py-0.5 text-[10.5px] font-semibold text-red-700 bg-red-100 rounded-full">NON-AKTIF</span>
              </td>
              <td v-if="canWrite" class="px-3 py-2.5 text-right space-x-2">
                <button @click="openEditModal(t)" class="px-2.5 py-1 text-[11px] font-semibold text-blue-600 hover:text-blue-800 bg-blue-50 rounded-lg cursor-pointer">Edit</button>
                <button @click="deleteType(t)" class="px-2.5 py-1 text-[11px] font-semibold text-red-600 hover:text-red-800 bg-red-50 rounded-lg cursor-pointer">Hapus</button>
              </td>
            </tr>
          </tbody>
        </table>
