    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-white p-3.5 sm:p-4 md:p-5 rounded-xl border border-[#E5E7EB] shadow-xs">
        ...
        <div class="flex items-center gap-2">
          <span class="px-2.5 py-0.5 text-xs font-semibold rounded-full border bg-blue-50 text-blue-700 border-blue-200">
      ...
      <div class="bg-white rounded-xl border border-[#E5E7EB] shadow-xs overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-left">
            <thead class="bg-[#F8FAFC] border-b text-[#475569] uppercase tracking-wider font-semibold text-[11.5px] select-none">
              <tr>
                <th @click="handleSort('nama_noo')" class="px-3 py-3 cursor-pointer hover:bg-slate-100 transition">
                ...
                <th @click="handleSort('branch_id')" class="px-3 py-3 cursor-pointer hover:bg-slate-100 transition">
                  <div class="flex items-center gap-1.5">
                ...
                <th @click="handleSort('stage_code')" class="px-3 py-3 cursor-pointer hover:bg-slate-100 transition">
                  <div class="flex items-center gap-1.5">
                ...
                <th @click="handleSort('submitted_at')" class="px-3 py-3 cursor-pointer hover:bg-slate-100 transition">
                  <div class="flex items-center gap-1.5">
            ...
            <tbody class="divide-y divide-gray-200 text-[12.5px] leading-[17px]">
              <tr v-for="item in sortedSubmissions" :key="item.request_id" class="hover:bg-gray-50/80 transition">
                <!-- DATA TOKO -->
                <td class="px-3 py-2.5">
                  <div class="font-bold text-[13px] text-gray-900">{{ item.nama_noo }}</div>
                  <div class="text-[11.5px] text-gray-500 mt-0.5 max-w-xs truncate">{{ item.alamat_noo }}</div>
                  <div class="text-[11px] text-gray-400 mt-0.5">
                    {{ item.kel_noo ? `${item.kel_noo}, ` : '' }}{{ item.kec_noo ? `${item.kec_noo}, ` : '' }}{{ item.kab_kota_noo }}
                  </div>
                  <div class="mt-1 flex items-center gap-1.5">
                    <span class="px-1.5 py-0.5 text-[10px] font-bold bg-gray-100 text-gray-700 rounded border">
                      {{ item.type_outlet_code }}
                    </span>
                    <span v-if="item.code_noo_principal" class="px-1.5 py-0.5 text-[10.5px] font-mono font-bold bg-emerald-100 text-emerald-800 rounded border border-emerald-300">
                ...
                <!-- HIRARKI CABANG -->
                <td class="px-3 py-2.5">
                  <div class="font-semibold text-gray-800 text-[12.5px]">{{ item.branch_id }} - {{ item.branch_name || 'Cabang' }}</div>
                  <div class="text-[11.5px] text-gray-500 mt-0.5">{{ item.salesman_name }} ({{ item.salesman_code }})</div>
                  <div class="text-[11px] text-gray-400 mt-0.5">Region: {{ item.region_code }}</div>
                </td>

                <!-- VISUAL STEP WORKFLOW -->
                <td class="px-3 py-2.5 min-w-[280px]">
                ...
                <td class="px-3 py-2.5 text-right">
                  <button
                    ...
                    class="px-2.5 py-1 text-[11px] font-semibold text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg transition flex items-center gap-1.5 ml-auto shadow-2xs cursor-pointer"
                  >
