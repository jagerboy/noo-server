      ...
      <div v-if="resetModalState.isOpen" class="fixed inset-0 min-h-screen min-w-full w-full h-full z-[999999] bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl relative my-auto">
          ...
          <div class="flex justify-end gap-2 pt-2 border-t">
            <button @click="closeResetModal" class="px-3.5 py-2 text-xs text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg">
              Batal
            </button>
            <button
              @click="submitReset"
              :disabled="resetForm.processing"
              class="px-4 py-2 text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-lg shadow-sm transition flex items-center gap-1 cursor-pointer"
            >
              <span>↩️ Ya, Lakukan Reset</span>
            </button>
          </div>
        </div>
      </div>
