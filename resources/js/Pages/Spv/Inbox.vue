...
<template>
  ...
  <SpvLayout>
    <div class="mx-auto max-w-7xl px-3 sm:px-4 md:px-6 lg:px-8 space-y-4 md:space-y-6">
      ...
      <!-- Header & Stats Counter Cards: Responsif Desktop & Tablet Samsung -->
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 md:gap-5">
        <div>
          <div class="flex items-center gap-2 sm:gap-3 flex-wrap">
            <h1 class="text-lg sm:text-xl md:text-[22px] font-bold text-[#111827] tracking-tight whitespace-nowrap">
              Inbox Submisi Toko
            </h1>
            ...
            <button
              ...
              class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl text-indigo-700 bg-indigo-50 hover:bg-indigo-100 active:bg-indigo-200 border border-indigo-200 transition cursor-pointer shadow-2xs shrink-0 whitespace-nowrap min-h-[34px]"
            ...
            <span class="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-[#F3E8FF] text-[#7E22CE] border border-[#C084FC] shrink-0 whitespace-nowrap">
              Verifikasi & Approval
            </span>
          </div>
          <p class="text-[12.5px] md:text-[13.5px] leading-[1.5] font-normal text-[#6B7280] mt-1">
            Verifikasi pendaftaran outlet baru, alokasi jadwal kunjungan sales (H1-H7 & M1-M4), dan persetujuan ke Principal.
          </p>
        </div>

        <!-- Metric Stat Badges (Klik untuk filter cepat, Ramah Tablet & Desktop) -->
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 w-full lg:w-auto shrink-0 select-none">
          <div
            @click="filterByMetric('pendingSpv')"
            class="p-3 sm:p-3.5 rounded-xl border shadow-xs text-center flex flex-col justify-between h-full min-w-[110px] md:min-w-[125px] cursor-pointer transition select-none hover:shadow-sm"
            :class="spvStatusFilter === 'PENDING' ? 'bg-blue-50/70 border-[#2563EB] ring-2 ring-[#2563EB]/25' : 'bg-white border-[#E5E7EB] hover:border-blue-300'"
            title="Klik untuk filter data yang belum diproses review SPV"
          >
            <div class="min-h-[22px] flex items-center justify-center">
              <span class="text-[10.5px] md:text-[11px] font-bold uppercase tracking-wider text-[#1D4ED8]">SPV Pending</span>
            </div>
            <div class="text-xl md:text-2xl font-black text-[#2563EB] mt-1">{{ stats.pendingSpv }}</div>
          </div>

          <div
            @click="filterByMetric('approvedSpv')"
            class="p-3 sm:p-3.5 rounded-xl border shadow-xs text-center flex flex-col justify-between h-full min-w-[110px] md:min-w-[125px] cursor-pointer transition select-none hover:shadow-sm"
            :class="(spvStatusFilter === 'APPROVED' || spvStatusFilter === 'PROCESSED') && edpStatusFilter === 'PENDING' ? 'bg-purple-50/70 border-[#9333EA] ring-2 ring-[#9333EA]/25' : 'bg-white border-[#E5E7EB] hover:border-purple-300'"
            title="Klik untuk filter data yang disetujui SPV & menunggu Principal"
          >
            <div class="min-h-[22px] flex items-center justify-center">
              <span class="text-[10.5px] md:text-[11px] font-bold uppercase tracking-wider text-[#7E22CE]">SPV Approved</span>
            </div>
            <div class="text-xl md:text-2xl font-black text-[#9333EA] mt-1">{{ stats.approvedSpv }}</div>
          </div>

          <div
            @click="filterByMetric('approvedEdp')"
            class="p-3 sm:p-3.5 rounded-xl border shadow-xs text-center flex flex-col justify-between h-full min-w-[110px] md:min-w-[125px] cursor-pointer transition select-none hover:shadow-sm"
            :class="edpStatusFilter === 'APPROVED' ? 'bg-emerald-50/70 border-[#16A34A] ring-2 ring-[#16A34A]/25' : 'bg-white border-[#E5E7EB] hover:border-emerald-300'"
            title="Klik untuk filter data yang telah disetujui Principal"
          >
            <div class="min-h-[22px] flex items-center justify-center">
              <span class="text-[10.5px] md:text-[11px] font-bold uppercase tracking-wider text-[#15803D]">Approved Principal</span>
            </div>
            <div class="text-xl md:text-2xl font-black text-[#16A34A] mt-1">{{ stats.approvedEdp }}</div>
          </div>

          <div
            @click="filterByMetric('rejected')"
            class="p-3 sm:p-3.5 rounded-xl border shadow-xs text-center flex flex-col justify-between h-full min-w-[110px] md:min-w-[125px] cursor-pointer transition select-none hover:shadow-sm"
            :class="edpStatusFilter === 'REJECTED' || spvStatusFilter === 'REJECTED' ? 'bg-rose-50/70 border-[#DC2626] ring-2 ring-[#DC2626]/25' : 'bg-white border-[#E5E7EB] hover:border-rose-300'"
            title="Klik untuk filter data yang ditolak"
          >
            <div class="min-h-[22px] flex items-center justify-center">
              <span class="text-[10.5px] md:text-[11px] font-bold uppercase tracking-wider text-[#B91C1C]">Ditolak</span>
            </div>
            <div class="text-xl md:text-2xl font-black text-[#DC2626] mt-1">{{ stats.rejected }}</div>
          </div>
        </div>
      </div>

      <!-- Toolbar Pencarian & Multi-Filter: Responsif Tablet Samsung & Desktop -->
      <div class="relative bg-white p-3 sm:p-4 rounded-xl border border-[#E5E7EB] shadow-xs">
        <div class="flex flex-wrap items-center gap-2 sm:gap-3 w-full">
          
          <!-- Search Input -->
          <div class="relative flex-1 min-w-[200px] sm:min-w-[240px]">
            <svg class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[#9CA3AF]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <input
              ...
              class="w-full pl-9 pr-3 py-2 text-[12.5px] rounded-lg bg-white border border-[#D1D5DB] text-[#374151] placeholder-[#9CA3AF] focus:ring-2 focus:ring-blue-600 focus:border-blue-600 transition shadow-2xs"
            />
          </div>

          <!-- 1. Dropdown Cabang -->
          <div v-if="myBranches && myBranches.length > 0" class="flex items-center gap-1.5 shrink-0">
            <label class="text-[11.5px] font-semibold text-slate-500 whitespace-nowrap hidden sm:inline">Cabang:</label>
            <select
              v-model="branchFilter"
              @change="onFilterChange"
              class="pl-2.5 pr-8 py-2 text-[12px] font-medium rounded-lg bg-white border border-[#D1D5DB] text-[#1F2937] focus:ring-2 focus:ring-blue-600 focus:border-blue-600 shadow-2xs cursor-pointer min-w-[140px] max-w-[200px] truncate"
            >
          ...
          <div class="flex items-center gap-1.5 shrink-0">
            <label class="text-[11.5px] font-semibold text-slate-500 whitespace-nowrap hidden sm:inline">Review:</label>
            <select
              v-model="spvStatusFilter"
              @change="onFilterChange"
              class="pl-2.5 pr-8 py-2 text-[12px] font-medium rounded-lg bg-white border border-[#D1D5DB] text-[#1F2937] focus:ring-2 focus:ring-blue-600 focus:border-blue-600 shadow-2xs cursor-pointer min-w-[135px]"
            >
          ...
          <div class="flex items-center gap-1.5 shrink-0">
            <label class="text-[11.5px] font-semibold text-slate-500 whitespace-nowrap hidden sm:inline">Principal:</label>
            <select
          ...
          <!-- 4. Dropdown Urutkan -->
          <div class="flex items-center gap-1.5 shrink-0">
            <label class="text-[11.5px] font-semibold text-slate-500 whitespace-nowrap hidden sm:inline">Urutkan:</label>
            <select
              v-model="sortSelect"
              @change="onFilterChange"
              class="pl-2.5 pr-8 py-2 text-[12px] font-medium rounded-lg bg-white border border-[#D1D5DB] text-[#1F2937] focus:ring-2 focus:ring-blue-600 focus:border-blue-600 shadow-2xs cursor-pointer min-w-[145px]"
            >
          ...
          <!-- Reset Filter -->
          <div v-if="hasActiveFilter" class="ml-auto flex items-center">
            <button
              type="button"
              @click="resetAllFilters"
              class="inline-flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-semibold text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-lg border border-rose-200 transition cursor-pointer"
      ...
      <!-- TABEL SUBMISI OUTLET: Responsif Tablet Samsung & Desktop -->
      <div class="bg-white rounded-xl border border-[#E5E7EB] shadow-xs overflow-hidden">
        <div class="w-full overflow-x-auto">
          <table class="w-full text-left text-[12.5px] leading-[17px] text-[#374151] min-w-[880px]">
            <thead class="bg-[#F8FAFC] text-[11.5px] font-semibold text-[#475569] border-b border-[#E2E8F0] select-none">
              <tr>
                <th class="w-[25%] px-3.5 py-3">Toko & Salesman</th>
                <th class="w-[23%] px-3 py-3">Alamat & Cabang</th>
                <th class="w-[14%] px-3 py-3">Status</th>
                <th class="w-[11%] px-2.5 py-3">Cust Dist.</th>
                <th class="w-[11%] px-2.5 py-3">Cust Principal</th>
                <th class="w-[10%] px-2.5 py-3">Rute Kunjungan</th>
                <th class="w-[6%] px-3 py-3 text-center">Aksi</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-[#E2E8F0]">
              ...
              >
                ...
                <td class="px-3.5 py-3 align-middle">
                  ...
                  <div class="text-[11.5px] text-slate-600 mt-0.5">
                    {{ item.salesman_name || '-' }}
                  </div>
                </td>
                ...
                <td class="px-3 py-3 align-middle text-center">
                  <button
                    ...
                    class="inline-flex items-center justify-center px-3 py-1.5 text-[11.5px] font-bold text-white bg-blue-600 hover:bg-blue-700 active:bg-blue-800 rounded-lg shadow-2xs transition cursor-pointer shrink-0 whitespace-nowrap min-h-[30px]"
                  >
                    Kelola
                  </button>
                </td>
    ...
    <Teleport to="body">
      <div v-if="showDetailModal && selectedSubmission" class="fixed inset-0 min-h-screen min-w-full w-full h-full z-[99990] overflow-y-auto bg-black/60 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 md:p-6" @click.self="closeDetailModal">
        <div class="bg-white rounded-2xl max-w-4xl w-full max-h-[92vh] sm:max-h-[88vh] flex flex-col shadow-2xl border border-[#E5E7EB] overflow-hidden text-[#374151] my-auto">
          
          <!-- Header Modal: Responsif Tablet & Desktop -->
          <div class="px-4 sm:px-6 py-3.5 sm:py-4 bg-[#1E3A8A] text-white flex items-center justify-between shrink-0 shadow-xs">
            <div class="space-y-0.5">
              <div class="flex items-center space-x-2 sm:space-x-3 flex-wrap gap-1">
                <h3 class="text-base sm:text-lg md:text-[20px] font-bold leading-snug text-white">{{ selectedSubmission.nama_noo }}</h3>
                <span class="px-2 py-0.5 rounded text-[10.5px] sm:text-xs font-bold bg-white/20 text-white border border-white/30">
                  {{ selectedSubmission.type_outlet_code }} - {{ selectedSubmission.type_outlet_desc }}
                </span>
              </div>
              <p class="text-[11px] sm:text-xs text-blue-200">Request ID: {{ selectedSubmission.request_id }} | Branch: {{ selectedSubmission.branch_name }}</p>
            </div>
            <button @click="showDetailModal = false" class="text-blue-200 hover:text-white text-xl font-bold p-1.5 hover:bg-blue-800 rounded-lg cursor-pointer transition">
              ✕
            </button>
          </div>

          <!-- Body Modal Detail -->
          <div class="p-3.5 sm:p-5 md:p-6 space-y-4 sm:space-y-6 overflow-y-auto flex-1 bg-[#F8FAFC]">

            <!-- Info Grid Lapang Komprehensif -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4 bg-white p-4 sm:p-5 rounded-xl border border-[#E5E7EB] shadow-2xs">
              <div>
                <p class="text-[11px] sm:text-[11.5px] font-bold text-[#4B5563] uppercase tracking-wider">Pemilik Outlet & No. HP</p>
                <p class="text-[13.5px] sm:text-[14px] font-semibold text-[#111827] mt-0.5">
              ...
              <div>
                <p class="text-[11px] sm:text-[11.5px] font-bold text-[#4B5563] uppercase tracking-wider">Salesman & Waktu Submisi</p>
                <p class="text-[13.5px] sm:text-[14px] font-semibold text-[#111827] mt-0.5">
                  {{ selectedSubmission.salesman_name }} ({{ selectedSubmission.salesman_code }})
                </p>
                <p class="text-[11.5px] text-[#6B7280] mt-0.5">
                  {{ formatDate(selectedSubmission.submitted_at || selectedSubmission.created_at) }}
                </p>
              </div>

              <div>
                <p class="text-[11px] sm:text-[11.5px] font-bold text-[#4B5563] uppercase tracking-wider">Kode Cust Distributor</p>
                <p class="text-[13.5px] sm:text-[14px] font-mono font-semibold text-[#1D4ED8] mt-0.5">
                  {{ selectedSubmission.custcode_distributor || 'Belum diisi Admin' }}
                </p>
              </div>

              <div>
                <p class="text-[11px] sm:text-[11.5px] font-bold text-[#4B5563] uppercase tracking-wider">Status Workflow</p>
                <div class="flex items-center space-x-2 mt-0.5">
                  <span class="px-2.5 py-0.5 rounded-full text-[11.5px] font-semibold border" :class="getStatusBadgeStyle(selectedSubmission.status)">
              ...
              <div>
                <p class="text-[11px] sm:text-[11.5px] font-bold text-[#4B5563] uppercase tracking-wider">Customer Code Principal</p>
                <div class="mt-1">
                  <span
                    v-if="selectedSubmission.code_noo_principal"
                    class="px-2.5 py-1 text-[13.5px] font-mono font-bold text-[#065F46] bg-[#D1FAE5] border border-[#6EE7B7] rounded-md inline-block"
                  >
                    {{ selectedSubmission.code_noo_principal }}
                  </span>
                  <span v-else class="text-[12.5px] text-[#94A3B8] italic">
              ...
              <div>
                <p class="text-[11px] sm:text-[11.5px] font-bold text-[#4B5563] uppercase tracking-wider">Alamat Lengkap & Wilayah</p>
                <p class="text-[13px] sm:text-[13.5px] text-[#374151] mt-0.5">{{ selectedSubmission.alamat_noo }}</p>
                <p class="text-[11.5px] text-[#6B7280] mt-0.5">Kel. {{ selectedSubmission.kel_noo }}, Kec. {{ selectedSubmission.kec_noo }}, {{ selectedSubmission.kab_kota_noo }}, {{ selectedSubmission.provinsi_noo }}</p>
              </div>

              <div class="sm:col-span-2">
                <p class="text-[11px] sm:text-[11.5px] font-bold text-[#4B5563] uppercase tracking-wider">GPS Koordinat</p>
                <p class="text-[13px] sm:text-[13.5px] font-mono text-[#374151] mt-0.5">{{ selectedSubmission.la }}, {{ selectedSubmission.lg }}</p>
                <p class="text-[11.5px] text-[#15803D] font-semibold mt-0.5">Akurasi GPS: {{ selectedSubmission.accuracy_m }} meter</p>
              </div>
            </div>

            <!-- SECTION 4: TRACK RECORD PERSETUJUAN (TIMELINE PROGRESS TRACKER) -->
            <div class="bg-white p-4 sm:p-5 rounded-xl border border-[#E5E7EB] shadow-2xs space-y-4">
              <div class="flex items-center justify-between border-b border-[#F3F4F6] pb-3 flex-wrap gap-2">
                <h4 class="text-[13px] sm:text-[13.5px] font-bold text-[#111827] uppercase tracking-wider flex items-center gap-2">
              ...
              <div class="relative pl-7 sm:pl-9 space-y-4 pt-1 pb-1 before:absolute before:left-3.5 sm:before:left-4.5 before:top-4 before:bottom-4 before:w-0.5 before:bg-slate-200">
                ...
                <div class="relative flex items-start">
                  ...
                  <!-- Step Content Card -->
                  <div class="w-full bg-slate-50/80 p-3.5 sm:p-4 rounded-xl border border-slate-200/90 shadow-2xs hover:border-blue-300 transition-all space-y-2.5">
                    <div class="flex items-center justify-between flex-wrap gap-2 border-b border-slate-200/70 pb-2">
                      <div class="flex items-center gap-2 flex-wrap">
                        <span class="font-bold text-slate-800 text-[13px] sm:text-[13.5px]">1. Admin Distributor</span>
                      ...
                      <span class="text-[10.5px] sm:text-[11px] font-medium text-slate-700 bg-white px-2.5 py-1 rounded-md border border-slate-200 shadow-2xs">
                ...
                <div class="relative flex items-start">
                  ...
                  <!-- Step Content Card -->
                  <div class="w-full bg-slate-50/80 p-3.5 sm:p-4 rounded-xl border border-slate-200/90 shadow-2xs hover:border-purple-300 transition-all space-y-2.5">
                    <div class="flex items-center justify-between flex-wrap gap-2 border-b border-slate-200/70 pb-2">
                      <div class="flex items-center gap-2 flex-wrap">
                        <span class="font-bold text-slate-800 text-[13px] sm:text-[13.5px]">2. SPV Area</span>
                ...
                <div class="relative flex items-start">
                  ...
                  <!-- Step Content Card -->
                  <div class="w-full bg-slate-50/80 p-3.5 sm:p-4 rounded-xl border border-slate-200/90 shadow-2xs hover:border-emerald-300 transition-all space-y-2.5">
                    <div class="flex items-center justify-between flex-wrap gap-2 border-b border-slate-200/70 pb-2">
                      <div class="flex items-center gap-2 flex-wrap">
                        <span class="font-bold text-slate-800 text-[13px] sm:text-[13.5px]">3. EDP Principal</span>
            ...
            <!-- SECTION PREVIEW MAPS -->
            <div class="space-y-3 bg-[#F8FAFC] p-4 rounded-xl border border-[#E5E7EB]">
              <div class="flex items-center justify-between flex-wrap gap-2">
                <h3 class="text-[13px] sm:text-[13.5px] font-bold text-[#1F2937] uppercase tracking-wider flex items-center gap-2">
                  <span>🌐 Preview Peta Lokasi Toko</span>
                </h3>
                <a
                  :href="`https://www.google.com/maps?q=${selectedSubmission.la},${selectedSubmission.lg}`"
                  target="_blank"
                  class="text-[12.5px] font-semibold text-[#2563EB] hover:text-[#1D4ED8] flex items-center gap-1"
                >
              ...
              <div class="w-full h-64 sm:h-72 bg-[#E5E7EB] rounded-xl overflow-hidden border border-[#D1D5DB] relative shadow-inner">
                <iframe
            ...
            <!-- BERKAS FOTO TOKO (3 KOLEKSI HD): Responsif Tablet & Desktop -->
            <div class="bg-white p-4 sm:p-5 rounded-xl border border-[#E5E7EB] shadow-2xs space-y-4">
              <h4 class="text-[13px] font-bold text-[#111827] uppercase tracking-wider border-b border-[#F3F4F6] pb-2">
                📸 BERKAS FOTO TOKO & KTP (HD 1:1)
              </h4>

              <div class="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-4">
                <!-- Foto Depan -->
                <div class="bg-white p-2.5 rounded-xl border border-[#E5E7EB] text-center relative select-none shadow-2xs">
                  <span class="text-[11.5px] font-bold text-[#4B5563] block mb-2">1. FOTO DEPAN</span>
                  <div v-if="getPhotoUrl(selectedSubmission.photo_depan_url || selectedSubmission.photo_depan_path)" class="relative group cursor-pointer overflow-hidden rounded-lg" @click="activePhotoZoom = getPhotoUrl(selectedSubmission.photo_depan_url || selectedSubmission.photo_depan_path)">
                    <img
                      ...
                      alt="Foto Depan"
                      class="w-full h-40 sm:h-44 object-cover rounded-lg shadow-md pointer-events-none select-none"
                      draggable="false"
                ...
                <!-- Foto Dalam -->
                <div class="bg-white p-2.5 rounded-xl border border-[#E5E7EB] text-center relative select-none shadow-2xs">
                  <span class="text-[11.5px] font-bold text-[#4B5563] block mb-2">2. FOTO DALAM</span>
                ...
                <!-- Foto KTP -->
                <div class="bg-white p-2.5 rounded-xl border border-[#E5E7EB] text-center relative select-none shadow-2xs">
                  <span class="text-[11.5px] font-bold text-[#4B5563] block mb-2">3. FOTO KTP</span>
            ...
            <div class="bg-white p-4 sm:p-5 rounded-xl border border-[#93C5FD] shadow-xs space-y-4 sm:space-y-5">
              <div class="flex items-center justify-between border-b border-[#E2E8F0] pb-3 flex-wrap gap-2">
                <h4 class="text-[13.5px] sm:text-[14px] font-bold text-[#1D4ED8] uppercase tracking-wider flex items-center gap-2">
              ...
              <!-- Button-Only Hari Kunjungan (H1-H7) - Touch area ramah tablet & desktop -->
              <div>
                <label class="block text-[13px] sm:text-[13.5px] font-semibold text-[#4B5563] mb-2">
                ...
                <div class="grid grid-cols-4 sm:grid-cols-7 gap-2">
                  <button
                    ...
                    :class="[
                      'py-2.5 px-2 rounded-xl text-xs font-semibold border transition-all flex flex-col items-center justify-center space-y-0.5 min-h-[46px] cursor-pointer',
                      approveForm.h1 === 'Y'
              ...
              <div class="space-y-2 pt-2 border-t border-slate-100">
                <label class="block text-[13px] sm:text-[13.5px] font-bold text-[#1F2937]">
                  Periode Frekuensi Kunjungan Salesman (JKS): <span class="text-rose-500">*</span>
                </label>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <button
                    ...
                    :disabled="isReadOnly"
                    :class="[
                      'p-3 sm:p-3.5 rounded-xl border text-left transition-all flex items-center justify-between cursor-pointer min-h-[48px]',
                  ...
                  >
                    <div class="text-xs font-bold uppercase tracking-wider flex items-center gap-2">
                      <span class="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0" :class="selectedVisitPattern === 'F2' ? 'border-purple-600 bg-purple-600' : 'border-slate-400'">
                  ...
                  >
                    <div class="text-xs font-bold uppercase tracking-wider flex items-center gap-2">
                      <span class="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0" :class="selectedVisitPattern === 'F4' ? 'border-purple-600 bg-purple-600' : 'border-slate-400'">
              ...
              <!-- Button-Only Pola Minggu (M1-M4) -->
              <div class="space-y-2">
                <div class="flex items-center justify-between flex-wrap gap-1">
                  <label class="block text-[13px] sm:text-[13.5px] font-semibold text-[#4B5563]">
                  ...
                  <span v-else-if="selectedVisitPattern === 'F2'" class="text-[11px] font-bold text-purple-700 bg-purple-50 px-2 py-0.5 rounded border border-purple-200">
                    <template v-if="f2SelectedType === 'GANJIL'">Pilihan anda Minggu Ganjil (M1, M3)</template>
                    <template v-else-if="f2SelectedType === 'GENAP'">Pilihan anda Minggu Genap (M2, M4)</template>
                    <template v-else>Klik M1/M3 (ganjil) atau M2/M4 (genap)</template>
                  </span>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3">
                  <button
                    ...
                    :class="[
                      'py-2.5 px-3 rounded-xl text-xs font-semibold border transition-all flex items-center justify-center space-x-1.5 min-h-[44px] cursor-pointer',
                      approveForm.m1 === 'Y'
              ...
              <!-- Catatan SPV Area -->
              <div>
                <label class="block text-[13px] sm:text-[13.5px] font-semibold text-[#4B5563] mb-1">
                ...
                <textarea
                  ...
                  :class="[
                    'w-full px-3.5 py-2 text-[13.5px] font-normal rounded-xl bg-white border text-[#374151] focus:ring-2 focus:ring-[#3B82F6]',
          ...
          <!-- Footer Action Modal: Touch-friendly buttons -->
          <div class="px-4 sm:px-6 py-3 sm:py-4 bg-[#F1F5F9] border-t border-[#E5E7EB] flex items-center justify-between flex-wrap gap-2 shrink-0">
            <button
              @click="showDetailModal = false"
              class="px-4 sm:px-5 py-2 sm:py-2.5 text-[13px] sm:text-[14px] font-semibold text-[#4B5563] bg-white border border-[#D1D5DB] rounded-xl hover:bg-gray-50 transition cursor-pointer min-h-[40px]"
            >
              Tutup
            </button>

            <div v-if="selectedSubmission.status === 'PUSHED_TO_SPV'" class="flex items-center space-x-2.5 sm:space-x-3">
              <button
                @click="openRejectModal(selectedSubmission)"
                class="px-4 sm:px-5 py-2 sm:py-2.5 text-[13px] sm:text-[14px] font-bold text-white bg-[#DC2626] hover:bg-[#B91C1C] rounded-xl transition shadow-xs cursor-pointer min-h-[40px]"
              >
                Rejected
              </button>
              ...
              <button
                ...
                class="px-5 sm:px-6 py-2 sm:py-2.5 text-[13px] sm:text-[14px] font-bold text-white bg-[#10B981] hover:bg-[#059669] active:bg-[#047857] rounded-xl transition shadow-xs disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 cursor-pointer min-h-[40px]"
              >
